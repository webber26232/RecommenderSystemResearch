...
def pearson(n_x, yr, min_support):
    ...
    cdef np.ndarray[np.float32_t, ndim=2] sqi
    # sum (rx'y ^ 2) for common ys
    cdef np.ndarray[np.float32_t, ndim=2] sqj
    # sum (rxy) for common ys
    cdef np.ndarray[np.float32_t, ndim=2] si
    # sum (rx'y) for common ys
    cdef np.ndarray[np.float32_t, ndim=2] sj
    # the similarity matrix
    cdef np.ndarray[np.float32_t, ndim=2] sim

    cdef int xi, xj
    cdef np.float32_t ri, rj
    cdef np.int32_t min_sprt, n
    min_sprt = min_support

    freq = np.zeros((n_x, n_x), np.int32)
    prods = np.zeros((n_x, n_x), np.float32)
    sqi = np.zeros((n_x, n_x), np.float32)
    sqj = np.zeros((n_x, n_x), np.float32)
    si = np.zeros((n_x, n_x), np.float32)
    sj = np.zeros((n_x, n_x), np.float32)
